...
const AIR_METRICS = [
  { key: 'temp',     label: 'TEMPERATURE', big: true  },
  { key: 'humidity', label: 'HUMIDITY',    big: false },
  { key: 'co2',      label: 'CO₂',         big: false },
  { key: 'voc',      label: 'VOC INDEX',   big: false },
]

const TEMP_METRICS = [
  { key: 'temp',     label: 'TEMPERATURE', big: true  },
  { key: 'humidity', label: 'HUMIDITY',    big: false },
  { key: 'pressure', label: 'PRESSURE',    big: false },
  { key: '_comfort', label: 'COMFORT',     big: false },
]

export default function HeroStack({ room }: { room: Room }) {
  const metrics = room.kind === 'AIR' ? AIR_METRICS : TEMP_METRICS
  return (
...
function HeroTile({ room, metric, label, big }: {
  room: Room; metric: string; label: string; big: boolean
}) {
  const { p, a, units } = useTheme()
  const val = latest(room, metric)
  const isWarn = tone(metric, val) !== 'ok'
  const displayVal = metric === 'temp'
    ? toUnit(val, 'temp', units).toFixed(1)
    : METRIC_META[metric].fmt(val)
    ...
    <div className={`${styles.tile} ${big ? styles.tileBig : styles.tileSmall}`}>
      <div className={styles.tileLeft}>
        ...
        <div className={styles.tileValueRow}>
          <span className={big ? styles.valueBig : styles.valueSmall} style={{ color: valueColor }}>
            {displayVal}
          </span>
...
function ComfortTile({ room }: { room: Room }) {
  ...
  return (
    <div className={`${styles.tile} ${styles.tileSmall}`}>
      ...
      <div className={styles.tileRight}>
        <div className={styles.comfortWrap}>
          <div className={styles.comfortTrack}>
            <div className={styles.comfortFill} style={{ width: `${score}%`, background: a.hex }} />
          </div>
